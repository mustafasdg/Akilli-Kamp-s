...
export default function CreateCommunityScreen() {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      Alert.alert('Başarılı! 🎉', `"${trimmedName}" topluluğu oluşturuldu.`, [
        { text: 'Tamam', onPress: () => navigation.goBack() },
      ]);
    } catch (e: any) {
      Alert.alert('Hata', e?.response?.data?.message ?? 'Topluluk oluşturulamadı. Lütfen tekrar deneyin.');
    } finally {
      setSaving(false);
    }
  };
    ...
    <SafeAreaView style={s.safe} edges={['top']}>
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        ...
        <View style={s.header}>
          ...
          <Text style={s.headerTitle}>Yeni Topluluk</Text>
          <View style={{ width: 38 }} />
        </View>
          ...
          >
            {saving ? (
              ...
              <>
                ...
                <Text style={s.submitText}>Topluluğu Oluştur</Text>
              </>
            )}
          </TouchableOpacity>
        </View>
